...
一个面向 OpenAI 兼容图像生成 / 编辑接口的本地工作台，当前版本 **0.1.51**。它把
...
- **Responses 旧模型彻底退役**：修复 IndexedDB 工作区快照在设置迁移后再次写回 `gpt-5.5`；数据库升级到 schema v12，前端同步迁移本地设置和工作区，服务端对所有入口无条件把精确旧默认值归一化为 `gpt-5.6-sol`。
...
```bash
docker build -t minorli/picgen:0.1.51 .
docker run --rm -p 8000:8000 \
  -v picgen-data:/app/data \
  minorli/picgen:0.1.51
```
...
默认会构建并推送 `minorli/picgen:0.1.51`。也可以覆盖：

```bash
IMAGE=minorli/picgen VERSION=0.1.51 PLATFORM=linux/amd64 ./scripts/docker-build-push.sh
```
...
PicGen 0.1.51 默认把所有图像操作收敛到 **OpenAI Images API + `gpt-image-2`**：
