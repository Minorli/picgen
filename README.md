...
一个面向 OpenAI 兼容图像生成 / 编辑接口的本地工作台，当前版本 **0.1.39**。它把
...
- **保持上游实际图片尺寸**：用户选择的 `size` 会继续发给上游；若上游返回不同尺寸，PicGen 按上游实际尺寸保存和展示，不补白、不裁切、不重编码。
...
```bash
docker build -t minorli/picgen:0.1.39 .
docker run --rm -p 8000:8000 \
  -v picgen-data:/app/data \
  minorli/picgen:0.1.39
```
...
默认会构建并推送 `minorli/picgen:0.1.39`。也可以覆盖：

```bash
IMAGE=minorli/picgen VERSION=0.1.39 PLATFORM=linux/amd64 ./scripts/docker-build-push.sh
```
...
PicGen 0.1.39 默认把所有图像操作收敛到 **OpenAI Images API + `gpt-image-2`**：
